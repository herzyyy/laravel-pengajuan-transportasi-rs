...
<div class="mx-auto px-3 pt-3 pb-4" style="max-width:48rem;">
    ...
    <div class="d-flex align-items-center justify-content-between gap-2 mb-3">
        <div>
            <div class="d-flex align-items-center gap-2 flex-wrap">
                ...
                <span class="badge {{ $statusCfg[0] }} d-inline-flex align-items-center gap-1 rounded-pill text-xxs fw-bold">
                    <span class="rounded-circle" style="width:.375rem; height:.375rem; display:inline-block; background:currentColor; opacity:.7;"></span>
                    {{ $statusCfg[4] }}
                </span>
    ...
    <div class="sp-card px-3 py-2 mb-3">
        <div class="d-flex align-items-center position-relative">
            <div class="position-absolute bg-slate-200" style="left:.75rem; right:.75rem; top:.75rem; height:1px; z-index:0;"></div>
            @foreach(['diajukan'=>'Diajukan','diproses'=>'Disetujui','digunakan'=>'Digunakan','selesai'=>'Selesai'] as $step=>$stepLabel)
                ...
                <div class="position-relative d-flex flex-column align-items-center flex-grow-1" style="z-index:1;">
                    <div class="d-flex align-items-center justify-content-center rounded-circle fw-bold border-2"
                         style="width:1.5rem; height:1.5rem; font-size:.5625rem;
                    ...
                    <span class="text-xxs fw-500 mt-1 whitespace-nowrap {{ $done ? 'text-emerald-700' : 'text-slate-400' }}">{{ $stepLabel }}</span>
                </div>
            @endforeach
        </div>
    </div>
        ...
        <div class="d-flex flex-column gap-3">
            ...
            <div class="sp-card overflow-hidden">
                <div class="d-flex align-items-center gap-2 px-3 py-2 bg-slate-50 border-bottom border-slate-200">
                    <svg class="text-emerald-600" width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                ...
                <div class="px-3 py-1">
                    <table class="w-100 text-xs">
                        <tbody class="divide-y divide-slate-100">
                            <tr>
                                <td class="py-2 pe-3 text-slate-400 whitespace-nowrap" style="width:5rem;">Jenis</td>
                                <td class="py-2">
                                    <span class="badge {{ $item->jenis === 'ambulance' ? 'badge-red' : 'badge-amber' }} d-inline-flex align-items-center rounded-pill text-xxs fw-bold">
                                        {{ ucfirst($item->jenis) }}
                                    </span>
                                    @if($item->keperluan)
                            ...
                            <tr>
                                ...
                                <td class="py-2 fw-500 text-slate-800" style="font-size:.6875rem;">
                                    ...
                                    @if($item->tanggal_sampai && $item->jam_sampai)
                                        <span class="text-slate-400 fw-normal d-block">→ {{ $item->tanggal_sampai->format('d M Y') }}, {{ substr($item->jam_sampai, 0, 5) }}</span>
                                    @else
                            ...
                            <tr>
                                ...
                                <td class="py-2 text-slate-400 text-xxs">{{ $item->created_at->format('d M Y, H:i') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="sp-card overflow-hidden" style="border-color:#fecdd3;">
                <div class="d-flex align-items-center gap-2 px-3 py-2 border-bottom" style="background-color:#fff1f2; border-color:#fecdd3;">
                    <svg class="text-red-600" width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                    </svg>
                    <h2 class="text-xxs fw-bold text-uppercase tracking-wide mb-0" style="color:#be123c;">Data Pasien</h2>
                </div>
                <div class="px-3 py-1">
                    <table class="w-100 text-xs">
                        <tbody class="divide-y divide-slate-100">
        ...
        <div class="d-flex flex-column gap-3">
            ...
            <div class="sp-card overflow-hidden">
                <div class="d-flex align-items-center gap-2 px-3 py-2 bg-slate-50 border-bottom border-slate-200">
                    ...
                    <h2 class="text-xxs fw-bold text-slate-600 text-uppercase tracking-wide mb-0">Kendaraan &amp; Perjalanan</h2>
                </div>
                <div class="px-3 py-2">
                    <div class="row g-2 text-xs">
                        <div class="col-6">
                            <div class="bg-slate-50 rounded p-2 border border-slate-200">
                                ...
                                <p class="fw-bold text-slate-800 mb-0" style="font-size:.6875rem;">{{ $item->unit_mobil }}</p>
                                @if($item->plat_nomor)<p class="text-xxs text-slate-500 font-mono mb-0">{{ $item->plat_nomor }}</p>@endif
                            </div>
                        </div>
                        @if($item->driver)
                        <div class="col-6">
                            <div class="bg-slate-50 rounded p-2 border border-slate-200">
                                ...
                                <p class="fw-bold text-slate-800 mb-0" style="font-size:.6875rem;">{{ $item->driver->name }}</p>
                            </div>
                        </div>
                        @endif
                        @if($item->km_awal)
                        <div class="col-6">
                            <div class="bg-emerald-50 rounded p-2 border border-emerald-200">
                                ...
                                <p class="fw-bold text-emerald-800 mb-0" style="font-size:.6875rem;">{{ number_format($item->km_awal, 0, ',', '.') }} km</p>
                            </div>
                        </div>
                        @endif
                        @if($item->km_akhir)
                        <div class="col-6">
                            <div class="bg-emerald-50 rounded p-2 border border-emerald-200">
                                ...
                                <p class="fw-bold text-emerald-800 mb-0" style="font-size:.6875rem;">{{ number_format($item->km_akhir, 0, ',', '.') }} km</p>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="col-12">
                            <div class="bg-emerald-50 rounded p-2 border border-emerald-200 d-flex align-items-center gap-2">
                                <svg class="text-emerald-600 shrink-0" width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"/>
                                </svg>
                                <div>
                                    <p class="text-xxs text-emerald-600 fw-500 mb-0">Total Jarak</p>
                                    <p class="fw-bold text-emerald-800 mb-0" style="font-size:.6875rem;">{{ number_format($item->km_akhir - $item->km_awal, 0, ',', '.') }} km</p>
                                </div>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="col-12">
                            <div class="bg-slate-50 rounded p-2 border border-slate-200 d-flex align-items-center gap-2">
                                <svg class="text-slate-500 shrink-0" width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                </svg>
                                <div>
                                    <p class="text-xxs text-slate-400 fw-500 mb-0">Jam Kedatangan</p>
                                    <p class="fw-bold text-slate-800 mb-0" style="font-size:.6875rem;">{{ $item->jam_kedatangan }} WIB</p>
                                </div>
                            </div>
                        </div>
                        @endif
