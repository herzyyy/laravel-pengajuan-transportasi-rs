    ...
    <div class="container py-3" style="max-width:480px;">
        ...
        <div class="sp-card mb-3 px-4 py-3">
            <div class="d-flex align-items-center gap-3">
                ...
                <div class="flex-grow-1 min-w-0">
                    <div class="text-sm fw-700 text-slate-800 truncate">{{ $user->full_name }}</div>
                    <div class="text-xs text-slate-500 truncate">{{ $user->unit_kerja ?? '-' }}</div>
                    <span class="badge mt-1" style="background:rgba(0,119,116,0.1);color:#007774;font-size:.65rem;">
                        {{ $user->jabatan ?? $user->profesi ?? 'Karyawan' }}
                    </span>
                </div>
            </div>
            ...
            <div class="d-flex gap-2 mt-3">
                <div class="flex-fill text-center rounded py-2" style="background:#f8fafc;border:1px solid #e2e8f0;">
                    <div class="text-base fw-700" style="color:#007774;">{{ $totalPengajuan }}</div>
                    <div class="text-xxs text-slate-500">Pengajuan</div>
                </div>
                <div class="flex-fill text-center rounded py-2" style="background:#f8fafc;border:1px solid #e2e8f0;">
                    <div class="text-base fw-700 text-success">{{ $totalDisetujui }}</div>
                    <div class="text-xxs text-slate-500">Disetujui</div>
                </div>
                <div class="flex-fill text-center rounded py-2" style="background:#f8fafc;border:1px solid #e2e8f0;">
                    <div class="text-base fw-700 text-danger">{{ $totalDitolak }}</div>
                    <div class="text-xxs text-slate-500">Ditolak</div>
                </div>
            </div>
        </div>
        ...
        <div class="sp-card mb-3">
            <div class="px-3 py-2 border-bottom border-slate-100">
                <span class="text-xs fw-600 text-slate-600">Informasi Akun</span>
            </div>
            <div class="px-3">
                @php
                    $rows = [
                        ...
                        ['label' => 'Posisi',    'value' => $user->posisi_pekerjaan ?? '-'],
                        ['label' => 'Profesi',   'value' => $user->profesi ?? '-'],
