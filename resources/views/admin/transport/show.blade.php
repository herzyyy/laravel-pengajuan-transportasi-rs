    ...
    <div>
        <div class="d-flex align-items-center gap-2 flex-wrap">
            ...
            <span class="badge status-badge {{ $sc['badge'] }} d-inline-flex align-items-center gap-1">
                <span class="rounded-circle {{ $sc['dot'] }}" style="width:.375rem;height:.375rem;display:inline-block;"></span>
                {{ $label }}
            </span>
...
<div class="sp-card px-3 py-2 mb-3">
    <div class="d-flex align-items-center position-relative">
        <div class="position-absolute start-0 end-0" style="top:.75rem;height:1px;background:#e2e8f0;z-index:0;margin:0 .75rem;"></div>
        @foreach(['diajukan' => 'Diajukan', 'diproses' => 'Disetujui', 'digunakan' => 'Digunakan', 'selesai' => 'Selesai'] as $step => $stepLabel)
            ...
            <div class="position-relative d-flex flex-column align-items-center flex-grow-1" style="z-index:1;">
                <div class="d-flex align-items-center justify-content-center rounded-circle fw-bold"
                     style="width:1.5rem;height:1.5rem;font-size:.5625rem;border:2px solid;
                    ...
                    @if($done && !$active)
                        <svg style="width:.75rem;height:.75rem;" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                    @else
                        {{ $stepIdx + 1 }}
                    @endif
                </div>
                <span class="text-xxs fw-500 mt-1 whitespace-nowrap {{ $done ? 'text-teal-700' : 'text-slate-400' }}">{{ $stepLabel }}</span>
            </div>
        @endforeach
    </div>
</div>
...
<div class="col-12 col-lg-8">
    ...
    <div class="sp-card overflow-hidden mb-3">
        <div class="sp-card-header d-flex align-items-center gap-2">
            <svg style="width:.75rem;height:.75rem;" class="text-teal-700" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
            </svg>
            <h2 class="text-xxs fw-bold text-slate-600 text-uppercase tracking-wide mb-0">Informasi Pengajuan</h2>
        </div>
        <div class="px-3 py-2">
            <table class="w-100 text-xs">
                <tbody class="divide-y divide-slate-100">
                    <tr>
                        <td class="py-1 pe-3 text-slate-400 whitespace-nowrap" style="width:7rem;">Pemohon</td>
                        <td class="py-1 fw-500 text-slate-800">
                            {{ $transportRequest->user->full_name ?? $transportRequest->pemohon_nama }}
                            <span class="text-slate-400 fw-normal"> &middot; {{ $transportRequest->user->unit_kerja ?? $transportRequest->pemohon_unit }}</span>
                            @if($transportRequest->user->nip ?? null)
                                <span class="text-slate-400 font-mono fw-normal"> &middot; {{ $transportRequest->user->nip }}</span>
                            @endif
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="py-1">
                            <span class="badge status-badge {{ $transportRequest->jenis === 'ambulance' ? 'bg-red-100 text-red-700' : 'bg-teal-50 text-teal-700' }}">
                                {{ ucfirst($transportRequest->jenis) }}
                            </span>
                            @if($transportRequest->jenis === 'ambulance' && $transportRequest->keperluan)
                                <span class="text-slate-500 ms-1">({{ ucfirst($transportRequest->keperluan) }})</span>
                            @endif
                            @if($transportRequest->jenis === 'umum' && $transportRequest->jumlah_penumpang)
                                <span class="text-slate-500 ms-1">&middot; {{ $transportRequest->jumlah_penumpang }} penumpang</span>
                            @endif
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="py-1 fw-500 text-slate-800">
                            ...
                            @if($transportRequest->tanggal_sampai && $transportRequest->jam_sampai)
                                <span class="text-slate-400">&rarr;</span> {{ $transportRequest->tanggal_sampai->format('d M Y') }}, {{ substr($transportRequest->jam_sampai, 0, 5) }}
                            @else
                    ...
                    <tr>
                        ...
                        <td class="py-1 fw-500 text-slate-800">
                            ...
                            @if($transportRequest->alamat_asal)
                                <span class="text-slate-400 fw-normal"> (dari: {{ $transportRequest->alamat_asal }})</span>
                            @endif
                        </td>
                    </tr>
    ...
    <div class="sp-card overflow-hidden border-red-200 mb-3">
        <div class="sp-card-header d-flex align-items-center gap-2 bg-red-50 border-bottom border-red-200">
            <svg style="width:.75rem;height:.75rem;" class="text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
        ...
        <div class="px-3 py-2">
            <table class="w-100 text-xs">
                <tbody class="divide-y divide-slate-100">
                    <tr>
                        <td class="py-1 pe-3 text-slate-400 whitespace-nowrap" style="width:7rem;">Nama</td>
                        <td class="py-1 fw-500 text-slate-800">{{ $transportRequest->pasien_nama ?? '-' }}</td>
                    </tr>
                    <tr>
    ...
    <div class="sp-card overflow-hidden mb-3">
        <div class="sp-card-header d-flex align-items-center gap-2">
            <svg style="width:.875rem;height:.875rem;" class="text-teal-700" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
            ...
            <h2 class="text-xxs fw-bold text-slate-700 text-uppercase tracking-wide mb-0">Kendaraan &amp; Perjalanan</h2>
        </div>
        <div class="px-3 py-3">
            <div class="row g-2 text-xs">
                <div class="col-6 col-sm-3">
                    <div class="bg-slate-50 rounded p-2 border border-slate-200">
                        ...
                        <p class="fw-bold text-slate-800 mb-0" style="font-size:.6875rem;">{{ $transportRequest->unit_mobil ?? '-' }}</p>
                        @if($transportRequest->plat_nomor)
                            <p class="text-xxs text-slate-500 font-mono mb-0">{{ $transportRequest->plat_nomor }}</p>
                        @endif
                    </div>
                </div>
                <div class="col-6 col-sm-3">
                    <div class="bg-slate-50 rounded p-2 border border-slate-200">
                        ...
                        <p class="fw-bold text-slate-800 mb-0" style="font-size:.6875rem;">{{ $transportRequest->driver->name ?? '-' }}</p>
                        @if($transportRequest->driver?->phone)
                            <p class="text-xxs text-slate-500 mb-0">{{ $transportRequest->driver->phone }}</p>
                        @endif
                    </div>
                </div>
                <div class="col-6 col-sm-3">
                    <div class="bg-teal-50 rounded p-2 border border-cyan-200">
                        ...
                        <p class="fw-bold text-teal-800 mb-0" style="font-size:.6875rem;">{{ $transportRequest->km_awal ? number_format($transportRequest->km_awal, 0, ',', '.') : '-' }} km</p>
                    </div>
                </div>
                <div class="col-6 col-sm-3">
                    <div class="bg-teal-50 rounded p-2 border border-cyan-200">
                        <p class="text-xxs text-teal-600 fw-500 mb-1">KM Tiba</p>
                        <p class="fw-bold text-teal-800 mb-0" style="font-size:.6875rem;">{{ $transportRequest->km_akhir ? number_format($transportRequest->km_akhir, 0, ',', '.') : '-' }} km</p>
                    </div>
                </div>
                @if($transportRequest->km_awal && $transportRequest->km_akhir)
                <div class="col-12 col-sm-6">
                    <div class="bg-emerald-50 rounded p-2 border border-emerald-200 d-flex align-items-center gap-2">
                        <svg style="width:1.25rem;height:1.25rem;flex-shrink:0;" class="text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"/>
                        </svg>
                        <div>
                            <p class="text-xxs text-emerald-600 fw-500 mb-0">Total Jarak</p>
                            <p class="fw-bold text-emerald-800 mb-0">{{ number_format($transportRequest->km_akhir - $transportRequest->km_awal, 0, ',', '.') }} km</p>
                        </div>
                    </div>
                </div>
                @endif
                ...
                <div class="col-12 col-sm-6">
                    <div class="bg-slate-50 rounded p-2 border border-slate-200 d-flex align-items-center gap-2">
                        <svg style="width:1.25rem;height:1.25rem;flex-shrink:0;" class="text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                        <div>
                            <p class="text-xxs text-slate-400 fw-500 mb-0">Jam Kedatangan</p>
                            <p class="fw-bold text-slate-800 mb-0">{{ $transportRequest->jam_kedatangan }} WIB</p>
                        </div>
                    </div>
                </div>
                @endif
                ...
                <div class="col-12 col-sm-6">
                    <div class="bg-amber-50 rounded p-2 border border-amber-200 d-flex align-items-center gap-2">
                        <svg style="width:1.25rem;height:1.25rem;flex-shrink:0;" class="text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/>
                        </svg>
                        <div>
                            <p class="text-xxs text-amber-600 fw-500 mb-0">Biaya E-Tol</p>
                            <p class="fw-bold text-amber-800 mb-0">Rp {{ number_format($transportRequest->biaya_tol, 0, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
                @endif
...
<div class="col-12 col-lg-4">
    <div class="sp-card overflow-hidden sticky-top"
        ...
        <div class="d-flex align-items-center gap-2 px-3 py-2 border-bottom" style="background:#007774;">
            <svg style="width:.875rem;height:.875rem;color:#99f6e4;" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>
                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
            </svg>
            <h2 class="text-xxs fw-bold text-white text-uppercase tracking-wide mb-0">Eksekusi Admin</h2>
        </div>
        ...
        <form method="POST" action="{{ route('admin.transport.update', $transportRequest) }}"
            ...
            <div class="mb-3">
                <label class="d-block text-xxs fw-600 text-slate-600 mb-1 text-uppercase tracking-wide">Ubah Status</label>
                ...
                <div class="mt-2 d-flex align-items-start gap-2 rounded bg-red-50 border border-red-200 px-2 py-2">
                    <svg style="width:.875rem;height:.875rem;flex-shrink:0;margin-top:.1rem;" class="text-red-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                    <p class="text-xxs text-red-700 mb-0">Semua unit penuh di waktu ini. Hanya bisa ditolak.</p>
                </div>
                @endif
            </div>

            <div x-show="currentStatus === 'tidak_disetujui' && savedStatus === 'diajukan'" class="mb-3">
                <label class="d-block text-xxs fw-600 text-slate-600 mb-1 text-uppercase tracking-wide">Alasan Penolakan <span class="text-danger">*</span></label>
                <textarea name="rejection_reason" rows="3" class="form-control form-control-sm" placeholder="Tuliskan alasan...">{{ old('rejection_reason') }}</textarea>
                @error('rejection_reason')<p class="text-xxs text-danger mt-1 mb-0">{{ $message }}</p>@enderror
            </div>

            <div x-show="currentStatus === 'digunakan' && '{{ $transportRequest->status }}' === 'diproses'" class="mb-3">
                <div class="mb-2">
                    <label class="d-block text-xxs fw-600 text-slate-600 mb-1 text-uppercase tracking-wide">Unit Kendaraan <span class="text-danger">*</span></label>
                ...
                <div class="mb-2">
                    <label class="d-block text-xxs fw-600 text-slate-600 mb-1 text-uppercase tracking-wide">KM Keberangkatan <span class="text-danger">*</span></label>
            ...
            <div x-show="currentStatus === 'selesai' && '{{ $transportRequest->status }}' === 'digunakan'" class="mb-3">
                <div class="mb-2">
                    <label class="d-block text-xxs fw-600 text-slate-600 mb-1 text-uppercase tracking-wide">KM Tiba <span class="text-danger">*</span></label>
                    <input type="number" name="km_akhir" id="km_akhir" value="{{ old('km_akhir', $transportRequest->km_akhir) }}"
                           class="form-control form-control-sm" placeholder="Masukkan KM" min="0">
                </div>
                <div class="mb-2">
                    <label class="d-block text-xxs fw-600 text-slate-600 mb-1 text-uppercase tracking-wide">Jam Kedatangan <span class="text-danger">*</span></label>
            ...
            <div x-show="'{{ $transportRequest->status }}' === 'digunakan' || '{{ $transportRequest->status }}' === 'selesai'"
                 class="bg-slate-50 rounded border border-slate-200 p-3 mb-3" style="font-size:.6875rem;">
                <p class="text-xxs fw-bold text-slate-600 text-uppercase tracking-wide mb-2">Data Terisi</p>
                ...
                <div class="d-flex justify-content-between mb-1"><span class="text-slate-500">KM Akhir</span><span class="fw-600 text-slate-800">{{ number_format($transportRequest->km_akhir, 0, ',', '.') }} km</span></div>
                @endif
                @if($transportRequest->km_awal && $transportRequest->km_akhir)
                <div class="d-flex justify-content-between pt-1 border-top border-slate-200"><span class="text-slate-500">Total</span><span class="fw-bold text-emerald-700">{{ number_format($transportRequest->km_akhir - $transportRequest->km_awal, 0, ',', '.') }} km</span></div>
                @endif
                @if($transportRequest->jam_kedatangan)
                <div class="d-flex justify-content-between mt-1"><span class="text-slate-500">Jam Tiba</span><span class="fw-600 text-slate-800">{{ $transportRequest->jam_kedatangan }}</span></div>
                @endif
            </div>
        ...
        @if($transportRequest->status === 'digunakan')
        <div x-show="editOpen && currentStatus !== 'selesai'" x-transition class="px-3 pb-3 border-top border-slate-200 pt-3">
            <form method="POST" action="{{ route('admin.transport.update', $transportRequest) }}"
                ...
                <p class="text-xxs fw-bold text-slate-600 text-uppercase tracking-wide mb-2">Edit Data Digunakan</p>
                <div class="mb-2">
                    <label class="d-block text-xxs fw-600 text-slate-600 mb-1 text-uppercase tracking-wide">Unit Kendaraan</label>
                    <select name="unit_mobil" id="unit_mobil_edit" class="form-select form-select-sm">
                        ...
                        @if($transportRequest->unit_mobil && !$vehicles->contains('name', $transportRequest->unit_mobil))
                            <option value="{{ $transportRequest->unit_mobil }}" selected>{{ $transportRequest->unit_mobil }} &mdash; saat ini</option>
                        @endif
                    </select>
                ...
                <div class="mb-2">
                    ...
                    <select name="driver_id" class="form-select form-select-sm">
                        ...
                        @if($transportRequest->driver_id && !$drivers->contains('id', $transportRequest->driver_id))
                            <option value="{{ $transportRequest->driver_id }}" selected>{{ $transportRequest->driver->name ?? '-' }} &mdash; saat ini</option>
                        @endif
                    </select>
                </div>
                <div class="mb-2">
                    <label class="d-block text-xxs fw-600 text-slate-600 mb-1 text-uppercase tracking-wide">KM Keberangkatan</label>
                    <input type="text" id="km_awal_edit_display"
                           value="{{ $transportRequest->km_awal ? number_format($transportRequest->km_awal, 0, ',', '.') : '' }}"
                           class="form-control form-control-sm" placeholder="Masukkan KM" inputmode="numeric" autocomplete="off">
                    <input type="hidden" name="km_awal" id="km_awal_edit" value="{{ $transportRequest->km_awal }}">
                </div>
                <button type="submit" class="btn btn-sp-primary btn-sm w-100 fw-600">Simpan Perubahan</button>
            </form>
        </div>
        @endif
    </div>
