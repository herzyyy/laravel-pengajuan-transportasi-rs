<x-app-layout>
    <div class="max-w-7xl mx-auto px-6 pt-8 pb-12">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-slate-800">Master Kendaraan</h1>
                <p class="text-slate-500 mt-1 text-sm">Kelola data kendaraan transportasi</p>
            </div>
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-4 mb-4">
            <form action="{{ route('admin.vehicles.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
                <div class="flex-1">
                ...
                <div class="w-full sm:w-40">
                    <select name="type" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">
                ...
                <div class="flex gap-2">
                    ...
                    @if(request()->hasAny(['search', 'type', 'is_active']))
                        <a href="{{ route('admin.vehicles.index') }}" class="bg-slate-100 text-slate-600 px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-slate-200 transition border border-slate-200">
                            Reset
                        </a>
                    @endif
                </div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr class="text-xs font-semibold text-slate-600 uppercase tracking-wide">
                            ...
                            <th class="py-3 px-4 text-left">Plat Nomor</th>
                            <th class="py-3 px-4 text-left">Merk/Model</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($vehicles as $vehicle)
                            <tr class="hover:bg-slate-50 transition">
                                ...
                                <td class="py-3 px-4 text-slate-700 font-medium">{{ $vehicle->plate_number }}</td>
                                ...
                                <td class="py-3 px-4 text-slate-700">{{ $vehicle->year ?? '-' }}</td>
                                <td class="py-3 px-4">
