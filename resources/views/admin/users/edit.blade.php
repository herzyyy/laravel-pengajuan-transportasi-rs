<x-app-layout>
    <div class="max-w-2xl mx-auto px-3 sm:px-4 pt-3 pb-6">
        <div class="mb-3">
            <h1 class="text-lg font-bold text-slate-800">Edit User</h1>
            <p class="text-slate-500 mt-0.5 text-xs">Update data user</p>
        </div>

        <div class="bg-white rounded-xl shadow-sm ring-1 ring-slate-200 overflow-hidden">
            <form action="{{ route('admin.users.update', $user) }}" method="POST" class="p-4 space-y-3">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <select name="priority_level" required
                            ...
                            <option value="1" @selected(old('priority_level', $user->priority_level ?? 0) == 1)>Prioritas Tinggi</option>
                        </select>
                    </div>
                </div>
