            ...
            <div class="bg-slate-50 px-3 py-2.5 border-t border-slate-100 flex flex-col-reverse sm:flex-row gap-2 sm:justify-end">
                @if($item->status === 'diajukan' && !$item->signature_pemohon && $item->user_id === auth()->id())
                    <a href="{{ route('signature.show', $item) }}"
                       class="inline-flex justify-center items-center gap-1.5 px-3 py-2 border border-transparent text-xs font-semibold rounded-lg text-white bg-blue-600 hover:bg-blue-700 transition-colors w-full sm:w-auto">
